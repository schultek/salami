...
self.addEventListener("message", function(e) {
  ...
  var makeAreaFunc = function(layer) {
    if (layer.type=="rect") {
      layer.inArea = function(p) {
        return inArea(p, this.dimens, null);
      }
    } else if (layer.type=="ellipse") {
      layer.cx = layer.dimens.x+layer.dimens.w/2;
      layer.cy = layer.dimens.y+layer.dimens.h/2;
      layer.r2x = (layer.dimens.w/2)*(layer.dimens.w/2);
      layer.r2y = (layer.dimens.h/2)*(layer.dimens.h/2);
      layer.inArea = function(p) {
        return (((p.x-this.cx)*(p.x-this.cx))/this.r2x)+(((p.y-this.cy)*(p.y-this.cy))/this.r2y)<=1;
      }
    } else {
      layer.inArea = function(p) {
        return inArea(p, this.dimens, this.border);
      }
    }
  }
  ...
  toPix = function(pos) { //convert mm to pixel
    var p = {x: pos.x-image.dimens.x, y: pos.y-image.dimens.y};
    return {x: Math.round(p.x*image.pixels.shape[0]/image.dimens.w), y: Math.round(p.y*image.pixels.shape[1]/image.dimens.h)};
  }

  globals = getGlobalConstants();


  var apprLineCount = Math.max(layer.dimens.w, layer.dimens.h)/curve.gap;
  ...
  if (line0.length>0) {
    var left;
    for (var i=layer.render.lines.r<0?-layer.render.lines.r:1; layer.render.lines.fill||i<=layer.render.lines.l; i++) {
      left = getLine(-i);
      if (left.length==0) {
        break;
      }
    ...
    var right;
    for (var i=layer.render.lines.l<0?-layer.render.lines.l:1; layer.render.lines.fill||i<=layer.render.lines.r; i++) {
      right = getLine(i);
      if (right.length==0) {
        break;
      }
    ...
    var addLines = function(lines, delta, count, arrfunc) {
      ...
      var i=delta*2;
      while ((layer.render.lines.fill||Math.abs(i)<=count) && (prevline.length==0 || line.length>0)) {
        prevline = line;
        line = getLine(i);
        ...
        i += delta;
        if (layer.render.lines.fill&&Math.abs(i)>10000) {
          break;
        }
      }
    }

    var lcenter = {x: layer.dimens.x+layer.dimens.w/2, y: layer.dimens.y+layer.dimens.h/2};
...
function getPoint(step, cdata, widthdata) {
  ...
  if (layer.inArea(cp)) {
    ...
    var valid = forms.length>0?forms[forms.length-1].mask?-1:1:-1;
    for (var f = forms.length-1; f>=0; f--) {
      if (forms[f].inArea(cp)) {
        if (forms[f].mask && !forms[f].ownRenderer) {
          valid = 1;
        } else {
          valid = 0;
        }
      } else if (forms[f].mask && valid==-1) {
        valid = 0;
      }
    }
    if (valid==0) {
...
function getGlobalConstants() {
  if (curve.type=="Linie") {
    ...
    var {start, end, err} = makeBorderPoints(curve.dimens);
    if (err) return null;
...
function leftCloserToLayer(lcenter) {
  if (curve.type == "Linie" || curve.type == "Bogen") {
    var dright = dist(curve.dimens.x+curve.dcos*curve.gap, curve.dimens.y+curve.dsin*curve.gap, lcenter.x, lcenter.y);
    var dleft = dist(curve.dimens.x-curve.dcos*curve.gap, curve.dimens.y-curve.dsin*curve.gap, lcenter.x, lcenter.y);
    return dleft<dright;
  } else if (curve.type == "Kreis") {
    return dist(lcenter.x, lcenter.y, curve.dimens.x, curve.dimens.y)<globals.r;
  }
}
...
function generateGCode(lines) {
  ...
  var add = function(g, x, y, z, s) {
    var str = "G"+g+(x?" X"+x:"")+(y?" Y"+y:"")+(z?" Z"+z:"")+" F"+s;
    output.push(str);
    x = x!=null ? x : pos.x;
    y = y!=null ? y : pos.y;
    z = z!=null ? z : pos.z;
  ...
  add(1, machine.dimens.w, null, null, sOut);
  add(1, null, machine.dimens.h, null, sOut);
